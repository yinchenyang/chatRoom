...
int msg_handle(msg_t *msg,int connectfd)
{
    ...
    if(LOG == msg->action)
    {
        ...
        sprintf(sql_log,"insert into log_info(id,connectfd) values(%d,%d);",msg->id,connectfd); 
...
int main(void)
{
    ...
    sprintf(sql_create_log_info,"create table log_info(id INTEGER,connectfd INTEGER,primary key(id));");
