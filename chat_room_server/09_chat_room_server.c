...
void *read_write_pthread(void *arg)
{
	...
	sprintf(sql_create_record,"create table record(name TEXT,target TEXT,msg TEXT,time TEXT,primary key(name,time));");
	...
	while(1)
	{
		...
		else if(WHISPER == msg.action)
		{
			...
			sprintf(sql_whsp_record, "insert into record (name,target,msg,time) values('%s','%s','%s','%s');",msg.name,msg.target,msg.msg,pestime); 
		...
		else if(CHAT == msg.action)
		{
			...
			sprintf(sql_chat_record, "insert into record (name,target,msg,time) values('%s','%s','%s','%s');",msg.name,"all user",msg.msg,pestime); 
