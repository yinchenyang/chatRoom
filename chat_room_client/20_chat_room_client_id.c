...
void *thread_read(void *arg)
{
	...
	while(1)
	{
        ...
        printf("recieve msg msg.id = %d,msg.name = %s\n",msg.id,msg.name);
		switch(msg.action)
		{	
			case LOG_SUCC:
			{
				...
				//sprintf(sql_create_user_record,"create table %d(id INTEGER,name TEXT,size TEXT,target TEXT,msg TEXT,time TEXT,primary key(size,time));",msg.id);
